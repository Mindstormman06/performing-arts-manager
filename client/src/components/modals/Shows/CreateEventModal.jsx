...
export default function CreateEventModal({
	...
	const handleCreateEvent = async () => {
		...
		try {
			const startUtc = localDateAndTimeToUtcIso(
				formData.date,
				formData.start_time,
			);
								...
								<ModalBox>
									{showMembers.map((member) => (
										<ModalLabel
											key={member.users_id}
											variant="checkbox"
											htmlFor={`show-member-${member.users_id}`}
										>
											<ModalCheckbox
												id={`show-member-${member.users_id}`}
									...
									{unassignedCharacters.length > 0 ? (
										unassignedCharacters.map((character) => (
											<ModalLabel
												key={character.id}
												variant="checkbox"
												htmlFor={`show-character-${character.id}`}
											>
												<ModalCheckbox
													id={`show-character-${character.id}`}
