...
export default function ShowRoleModal({
	...
	const lockedHighestRoles = new Set(
		selectedRoles.filter(
			(roleName) => getRoleWeight(roleName) === highestSelectedRoleWeight,
		),
	);
	const memberId = resolvedMember?.users_id ?? user?.users_id ?? null;
	const isEditingSelf = Number(currentUserId) === Number(memberId);
	const normalizedCurrentUserRoles = currentUserRoles.map((role) =>
	...
	const maxAssignableRoleWeight = hasOrgLevelOverride
		? Number.POSITIVE_INFINITY
		: currentUserHighestRoleWeight;
	const baseAssignedRoles = (
	...
	const handleToggle = async (role) => {
		...
		if (
			...
			getRoleWeight(role) > highestBaseRoleWeight
		) {
		...
		if (
			!selectedRoles.includes(role) &&
			getRoleWeight(role) > maxAssignableRoleWeight
		) {
	...
	const memberPhotoUrl = memberPhotoPath
		? `${BACKEND_URL}${memberPhotoPath}`
		: "";
					...
					<div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_14rem]">
						<div>
								...
								{canEditRoles && (
									<div className="relative flex shrink-0 items-center">
										...
										{isEditingRoles && (
											<div
												ref={popoverRef}
												className="absolute top-1/2 left-full z-50 ml-3 -translate-y-1/2 rounded-lg border border-gray-200 bg-white shadow-xl"
												style={{ width: "448px" }}
											>
												<div className="rounded-t-lg border-gray-100 border-b bg-gray-50 px-3 py-2 font-semibold text-gray-600 text-xs">
													Assign Roles
												</div>
												<div className="grid max-h-48 w-full grid-cols-3 gap-2 overflow-y-auto p-2">
													{availableRoles.map((role) => {
														const isLockedHighestRole =
															selectedRoles.includes(role) &&
															lockedHighestRoles.has(role);
														const isHigherSelfPromotionRole =
															isEditingSelf &&
															!selectedRoles.includes(role) &&
															getRoleWeight(role) > highestBaseRoleWeight;
														const isHigherThanEditorRole =
															!selectedRoles.includes(role) &&
															getRoleWeight(role) > maxAssignableRoleWeight;
														...
														const disabledTitle = isLockedHighestRole
															...
															: isHigherSelfPromotionRole
																...
																: isHigherThanEditorRole
																	...
																	: undefined;
														const checkboxId = `show-role-${role
															.toLowerCase()
															.replace(/[^a-z0-9]+/gi, "-")}`;
														...
														return (
															...
															>
																...
																/>
																<span
																	className={`ml-2 text-sm capitalize ${isDisabledRole ? "text-gray-400" : "text-gray-700"}`}
																>
																	{role}
																</span>
															</ModalLabel>
								...
								<div className="flex flex-wrap gap-2 pt-0.5">
									{selectedRoles.length > 0 ? (
										selectedRoles.map((role) => (
											<span
												key={role}
												className="rounded-full bg-blue-600 px-3 py-1 font-semibold text-white text-xs capitalize"
											>
												{role}
											</span>
										))
									) : (
										<span className="rounded-full bg-gray-200 px-3 py-1 font-semibold text-gray-600 text-xs">
											No roles assigned
										</span>
									)}
								</div>
							...
							<div className="mt-4 space-y-1 text-black">
								<p className="text-lg">
									Email: {memberUser.email || "Not provided"}
								</p>
								<p className="text-lg">
									Phone: {memberUser.phone || "Not provided"}
								</p>
							</div>

							<div className="mt-5">
								<h3 className="mb-1 font-medium text-gray-700 text-xl">Bio</h3>
						...
						<div className="flex items-start justify-center md:justify-end">
							{memberPhotoUrl ? (
								...
								<div className="flex h-72 w-60 items-center justify-center rounded-2xl bg-gray-500 font-light text-6xl text-white">
									Photo
								</div>
							)}
						</div>
