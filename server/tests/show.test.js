...
describe("Show Management API", () => {
	...
	beforeAll(async () => {
		...
		const userRes = await request(app).post("/api/users").send({
			fname: "Show", lname: "Tester", email, password: "password123",
		});
	...
	describe("Show Controller Error Handling", () => {
		it("DELETE /api/shows/:id - should return 404 when show is not found", async () => {
			...
			const spy = vi.spyOn(showService, 'remove').mockRejectedValue(new Error("Show not found"));
		...
		it("POST /api/shows/:showId/join - should return 409 for user already in show", async () => {
			const spy = vi.spyOn(showMembershipService, 'addUserToShow').mockRejectedValue(new Error("User already in show"));
    ...
    describe("showController - Error Handling", () => {
		it("GET /api/shows - should handle generic errors", async () => {
			const spy = vi.spyOn(showService, 'getAll').mockRejectedValue(new Error("Some error"));
			const res = await request(app).get("/api/shows");
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		it("GET /api/shows/:id - should return 404 for show not found", async () => {
			const spy = vi.spyOn(showService, 'getById').mockRejectedValue(new Error("Show not found"));
			const res = await request(app).get(`/api/shows/999`);
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});
		it("GET /api/shows/:id - should handle generic errors", async () => {
			const spy = vi.spyOn(showService, 'getById').mockRejectedValue(new Error("Some other error"));
			const res = await request(app).get(`/api/shows/${testShowId}`);
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		it("POST /api/shows - should handle generic errors", async () => {
			const spy = vi.spyOn(showService, 'create').mockRejectedValue(new Error("Some error"));
			const res = await request(app).post("/api/shows").set("Authorization", `Bearer ${authToken}`).send({
					title: "New Show", start_date: "2026-09-01", end_date: "2026-09-15", organization_id: testOrgId,
			});
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		it("PUT /api/shows/:id - should return 404 for show not found", async () => {
			const spy = vi.spyOn(showService, 'update').mockRejectedValue(new Error("Show not found"));
			const res = await request(app).put(`/api/shows/${testShowId}`).set("Authorization", `Bearer ${authToken}`).send({ title: "Updated" });
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});
		it("PUT /api/shows/:id - should handle generic errors", async () => {
			const spy = vi.spyOn(showService, 'update').mockRejectedValue(new Error("Some other error"));
			const res = await request(app).put(`/api/shows/${testShowId}`).set("Authorization", `Bearer ${authToken}`).send({ title: "Updated" });
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		it("DELETE /api/shows/:id - should handle generic errors", async () => {
			const spy = vi.spyOn(showService, 'remove').mockRejectedValue(new Error("Some other error"));
			const res = await request(app).delete(`/api/shows/${testShowId}`).set("Authorization", `Bearer ${authToken}`);
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
        		it("POST /api/shows/:showId/join - should handle generic errors", async () => {
			const spy = vi.spyOn(showMembershipService, 'addUserToShow').mockRejectedValue(new Error("Some error"));
			const res = await request(app).post(`/api/shows/${testShowId}/join`).send({ userId: testUserId });
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		it("PUT /api/shows/:showId/users/:userId/roles - should return 404 for not found errors", async () => {
			const spy = vi.spyOn(showRoleService, 'appendRolesToAssignment').mockRejectedValue(new Error("not found"));
			const res = await request(app).put(`/api/shows/${testShowId}/users/${testUserId}/roles`).set("Authorization", `Bearer ${authToken}`).send({ roles: ["actor"] });
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});
		it("PUT /api/shows/:showId/users/:userId/roles - should handle generic errors", async () => {
			const spy = vi.spyOn(showRoleService, 'appendRolesToAssignment').mockRejectedValue(new Error("Some error"));
			const res = await request(app).put(`/api/shows/${testShowId}/users/${testUserId}/roles`).set("Authorization", `Bearer ${authToken}`).send({ roles: ["actor"] });
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		it("GET /api/shows/:showId/users - should return 404 for show not found", async () => {
			const spy = vi.spyOn(showRoleService, 'getShowUsers').mockRejectedValue(new Error("Show not found"));
			const res = await request(app).get(`/api/shows/${testShowId}/users`);
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});
		it("GET /api/shows/:showId/users - should handle generic errors", async () => {
			const spy = vi.spyOn(showRoleService, 'getShowUsers').mockRejectedValue(new Error("Some error"));
			const res = await request(app).get(`/api/shows/${testShowId}/users`);
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		it("GET /api/shows/:showId/users/:userId - should return 404 for not found", async () => {
			const spy = vi.spyOn(showRoleService, 'getShowUserById').mockRejectedValue(new Error("not found"));
			const res = await request(app).get(`/api/shows/${testShowId}/users/${testUserId}`);
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});
		it("GET /api/shows/:showId/users/:userId - should handle generic errors", async () => {
			const spy = vi.spyOn(showRoleService, 'getShowUserById').mockRejectedValue(new Error("Some error"));
			const res = await request(app).get(`/api/shows/${testShowId}/users/${testUserId}`);
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		it("GET /api/shows/:showId/users/search - should return 404 for not found", async () => {
			const spy = vi.spyOn(showRoleService, 'getUsersByRole').mockRejectedValue(new Error("not found"));
			const res = await request(app).get(`/api/shows/${testShowId}/users/search?role=actor`);
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});
		it("GET /api/shows/:showId/users/search - should handle generic errors", async () => {
			const spy = vi.spyOn(showRoleService, 'getUsersByRole').mockRejectedValue(new Error("Some error"));
			const res = await request(app).get(`/api/shows/${testShowId}/users/search?role=actor`);
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		it("DELETE /api/shows/:showId/users/:userId - should return 404 for not found", async () => {
			const spy = vi.spyOn(showRoleService, 'removeUserFromShow').mockRejectedValue(new Error("not a member"));
			const res = await request(app).delete(`/api/shows/${testShowId}/users/${testUserId}`);
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});
		it("DELETE /api/shows/:showId/users/:userId - should handle generic errors", async () => {
			const spy = vi.spyOn(showRoleService, 'removeUserFromShow').mockRejectedValue(new Error("Some error"));
			const res = await request(app).delete(`/api/shows/${testShowId}/users/${testUserId}`);
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		it("DELETE /api/shows/:showId/users/:userId/roles - should return 404 for not found", async () => {
			const spy = vi.spyOn(showRoleService, 'removeRolesFromUser').mockRejectedValue(new Error("not found"));
			const res = await request(app).delete(`/api/shows/${testShowId}/users/${testUserId}/roles`).set("Authorization", `Bearer ${authToken}`).send({ roles: ["actor"] });
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});
		it("DELETE /api/shows/:showId/users/:userId/roles - should handle generic errors", async () => {
			const spy = vi.spyOn(showRoleService, 'removeRolesFromUser').mockRejectedValue(new Error("Some error"));
			const res = await request(app).delete(`/api/shows/${testShowId}/users/${testUserId}/roles`).set("Authorization", `Bearer ${authToken}`).send({ roles: ["actor"] });
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
    ...
    describe("Auth Middleware Tests (Lines 33, 41, 54, 75, 87)", () => {
		...
		beforeAll(async () => {
			...
			const userRes = await request(app).post("/api/users").send({
				fname: "Basic", lname: "User", email: `basic-${Date.now()}@viu.ca`, password: "password123",
			});
		...
		it("authorizeOrg - should return 400 if orgId is missing (Line 33)", async () => {
			...
			const res = await request(app).post("/api/shows").set("Authorization", `Bearer ${authToken}`).send({
				title: "No Org Show", start_date: "2026-07-01", end_date: "2026-07-15"
			});
		...
		it("authorizeOrg - should return 403 if user is not a member of the org (Line 41)", async () => {
			...
			const res = await request(app).post("/api/shows").set("Authorization", `Bearer ${basicAuthToken}`).send({
				title: "Unauthorized Show", organization_id: testOrgId
			});
		...
		it("authorizeOrg - should return 403 for insufficient org permissions (Line 54)", async () => {
			...
			await request(app).post(`/api/orgs/${testOrgId}/join`).send({ userId: basicUserId });
			
			const res = await request(app).post("/api/shows").set("Authorization", `Bearer ${basicAuthToken}`).send({
				title: "No Admin Show", organization_id: testOrgId
			});
			expect(res.statusCode).toEqual(403);
		...
		it("authorizeShow - should return 403 if user is not a member of the show (Line 75)", async () => {
			...
			const res = await request(app).put(`/api/shows/${testShowId}`).set("Authorization", `Bearer ${basicAuthToken}`).send({
				title: "Hacked Show"
			});
			expect(res.statusCode).toEqual(403);
		...
		it("authorizeShow - should return 403 for insufficient show permissions (Line 87)", async () => {
			...
			await request(app).post(`/api/shows/${testShowId}/join`).send({ userId: basicUserId });
			await request(app).put(`/api/shows/${testShowId}/users/${basicUserId}/roles`).set("Authorization", `Bearer ${authToken}`).send({ roles: ["actor"] });
	...
	describe("showMembership.service.js - Direct Service Tests", () => {
		it("addUserToShow - should throw 'Show not found' (Line 5)", async () => {
			await expect(showMembershipService.addUserToShow(99999, testUserId))
				.rejects.toThrow("Show not found");
		});

		it("addUserToShow - should throw 'User not found' (Line 8)", async () => {
			await expect(showMembershipService.addUserToShow(testShowId, 99999))
		...
		it("addUserToShow - should throw 'User already in show' (Line 14)", async () => {
			...
			await expect(showMembershipService.addUserToShow(testShowId, testUserId))
	...
	describe("showRole.service.js - Direct Service Tests", () => {
		it("appendRolesToAssignment - should throw 'Show not found'", async () => {
			await expect(showRoleService.appendRolesToAssignment(99999, testUserId, ["actor"]))
				.rejects.toThrow("Show not found");
		});

		it("appendRolesToAssignment - should throw 'User is not a member'", async () => {
			await expect(showRoleService.appendRolesToAssignment(testShowId, 99999, ["actor"]))
				.rejects.toThrow("User is not a member of this show");
		});

		it("appendRolesToAssignment - should throw 'No valid roles'", async () => {
			await expect(showRoleService.appendRolesToAssignment(testShowId, testUserId, ["mythical-role"]))
				.rejects.toThrow("No valid roles provided");
		});

		it("getShowUsers - should throw 'Show not found'", async () => {
			await expect(showRoleService.getShowUsers(99999))
				.rejects.toThrow("Show not found");
		});
		...
		it("getShowUsers - should throw 'No users found'", async () => {
			...
			await expect(showRoleService.getShowUsers(emptyShow.id))
				.rejects.toThrow("No users found for this show");
		});

		it("getShowUserById - should throw 'Show not found'", async () => {
			await expect(showRoleService.getShowUserById(99999, testUserId))
				.rejects.toThrow("Show not found");
		});

		it("getShowUserById - should throw 'User not found in this show'", async () => {
			await expect(showRoleService.getShowUserById(testShowId, 99999))
				.rejects.toThrow("User not found in this show");
		});

		it("getUsersByRole - should throw 'Show not found'", async () => {
			await expect(showRoleService.getUsersByRole(99999, "actor"))
				.rejects.toThrow("Show not found");
		});

		it("getUsersByRole - should throw 'Role not found'", async () => {
			await expect(showRoleService.getUsersByRole(testShowId, "mythical-role"))
				.rejects.toThrow("Role not found");
		});

		it("removeUserFromShow - should throw 'Show not found'", async () => {
			await expect(showRoleService.removeUserFromShow(99999, testUserId))
				.rejects.toThrow("Show not found");
		});

		it("removeRolesFromUser - should throw 'Role names are required' (Line 95)", async () => {
			await expect(showRoleService.removeRolesFromUser(testShowId, testUserId, []))
		...
		it("removeUserFromShow - should throw 'User is not a member' (Line 95)", async () => {
			await expect(showRoleService.removeUserFromShow(testShowId, 99999))
				.rejects.toThrow("User is not a member of this show");
		});

		it("removeRolesFromUser - should throw 'Membership or Roles not found' (Lines 108-109)", async () => {
			// Fake user ID to trigger !membership
			await expect(showRoleService.removeRolesFromUser(testShowId, 99999, ["actor"]))
				.rejects.toThrow("Membership or Roles not found");
				
			// Fake role to trigger !roles.length
			await expect(showRoleService.removeRolesFromUser(testShowId, testUserId, ["mythical-role"]))
				.rejects.toThrow("Membership or Roles not found");
		});
		...
		it("removeRolesFromUser - should throw 'User does not have any of these roles' (Line 120)", async () => {
			...
			await expect(showRoleService.removeRolesFromUser(testShowId, testUserId, ["president"]))
		...
		it("removeRolesFromUser - should remove role successfully (Happy Path)", async () => {
			...
			await showRoleService.appendRolesToAssignment(testShowId, testUserId, ["actor"]);
			
			// Now remove it to hit the final return statement (Line 123)
			const res = await showRoleService.removeRolesFromUser(testShowId, testUserId, ["actor"]);
	...
	describe("show.service.js - Direct Service Tests", () => {
		...
		it("getById - should throw 'Show not found' for invalid ID", async () => {
			await expect(showService.getById(99999))
				.rejects.toThrow("Show not found");
		});

		it("update - should throw 'Show not found' (Line 26)", async () => {
			await expect(showService.update(99999, { title: "Fake" }))
				.rejects.toThrow("Show not found");
		});

		it("remove - should throw 'Show not found' (Lines 33-35)", async () => {
			await expect(showService.remove(99999))
				.rejects.toThrow("Show not found");
		});
