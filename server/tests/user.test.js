...
describe("User Management API", () => {
	...
	it("PUT /api/users/:id - should update user details", async () => {
		const res = await request(app).put(`/api/users/${testUserId}`).send({ fname: "Updated User" });
		expect(res.statusCode).toEqual(200);
	});
	...
	describe("User Controller - Error Handling", () => {
		...
		it("GET /api/users - should handle generic errors", async () => {
			const spy = vi.spyOn(userService, 'getAll').mockRejectedValue(new Error("Database error"));
			const res = await request(app).get("/api/users");
		...
		it("GET /api/users/:id - should return 404 for user not found", async () => {
			const spy = vi.spyOn(userService, 'getById').mockRejectedValue(new Error("User not found"));
		...
		it("PUT /api/users/:id - should return 404 for user not found", async () => {
			const spy = vi.spyOn(userService, 'update').mockRejectedValue(new Error("User not found"));
			const res = await request(app).put("/api/users/999").send({ fname: "Test" });
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});

		it("DELETE /api/users/:id - should return 404 for user not found", async () => {
			const spy = vi.spyOn(userService, 'remove').mockRejectedValue(new Error("User not found"));
        ...
        it("POST /api/users - should handle generic errors (Lines 27-28)", async () => {
			const spy = vi.spyOn(userService, 'create').mockRejectedValue(new Error("Database offline"));
			const res = await request(app).post("/api/users").send({ fname: "T", email: "t@t.com", password: "p" });
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});

		it("PUT /api/users/:id - should handle generic errors", async () => {
			const spy = vi.spyOn(userService, 'update').mockRejectedValue(new Error("Database offline"));
			const res = await request(app).put("/api/users/999").send({ fname: "Test" });
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		...
		it("DELETE /api/users/:id - should handle generic errors (Line 46)", async () => {
			...
			const spy = vi.spyOn(userService, 'remove').mockRejectedValue(new Error("Database offline"));
			const res = await request(app).delete("/api/users/999");
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
        ...
        it("GET /api/users/:id - should handle generic errors (Line 16 False Branch)", async () => {
			const spy = vi.spyOn(userService, 'getById').mockRejectedValue(new Error("Database error"));
			const res = await request(app).get("/api/users/999");
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
    ...
    it("DELETE /api/users/:id - should delete a user successfully (Line 46)", async () => {
		...
		const tempUser = await request(app).post("/api/users").send({
			fname: "Delete", lname: "Me", email: `delete-${Date.now()}@viu.ca`, password: "password123"
		});
	...
	describe("user.service.js - Direct Service Tests", () => {
		...
		it("update - should throw 'User not found' (Line 32)", async () => {
			await expect(userService.update(99999, { fname: "Ghost" }))
				.rejects.toThrow("User not found");
		});
		...
		it("update - should hash new password if provided (Lines 35-36)", async () => {
			...
			const updatedUser = await userService.update(testUserId, { password: "newpassword123" });
		...
		it("remove - should throw 'User not found' (Line 45)", async () => {
			await expect(userService.remove(99999))
				.rejects.toThrow("User not found");
		});
	});
});