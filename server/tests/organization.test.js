...
describe("Organization API & Permissions", () => {
	...
	beforeAll(async () => {
		...
		const userRes = await request(app).post("/api/users").send({
			fname: "Org", lname: "Tester", email, password: "password123",
		});
	...
	describe("Organization Core CRUD", () => {
		it("POST /api/orgs - should block creation if no token provided", async () => {
			const res = await request(app).post("/api/orgs").send({ name: "Test Org" });
	...
	describe("Org User Management (Happy Paths)", () => {
		...
		it("DELETE /api/orgs/:orgId/users/:userId - should remove user from organization (Line 96)", async () => {
			...
			const leaveUserRes = await request(app).post("/api/users").send({
				fname: "Leave", lname: "User", email: `leave-${Date.now()}@viu.ca`, password: "password123",
			});
			const leaveUserId = leaveUserRes.body.id;
			await request(app).post(`/api/orgs/${testOrgId}/join`).send({ userId: leaveUserId });
	...
	describe("Invitations", () => {
		let inviteeEmail = `invitee-${Date.now()}@viu.ca`;

		it("Should invite a user via email", async () => {
			// Create the user first so they exist in the DB
			await request(app).post("/api/users").send({
				fname: "Invitee", lname: "User", email: inviteeEmail, password: "password123",
			});
		...
		it("Should accept an organization invite", async () => {
			...
			const spy = vi.spyOn(models.OrgMembership, 'findOne').mockResolvedValue(mockMembership);
		...
		it("Should reject invite if user is already in org or pending (Service Line 43)", async () => {
			...
			expect(res.body.message).toContain("already in this organization or pending");
		});
	});
	...
	describe("Organization Error Handling Mocks", () => {
		...
		it("Should handle service crash during org creation", async () => {
			const spy = vi.spyOn(organizationService, 'create').mockRejectedValue(new Error("DB error"));
			const res = await request(app)
				.post("/api/orgs")
				.set("Authorization", `Bearer ${authToken}`)
				.send({ name: "Crash Org" });
	...
	describe("orgMembershipController - Error Handling", () => {
		...
		it("GET /api/orgs/:orgId/users/:userId - should handle generic errors", async () => {
			const spy = vi.spyOn(orgRoleService, 'getOrgUserById').mockRejectedValue(new Error("Some other error"));
			const res = await request(app)
				.get(`/api/orgs/${testOrgId}/users/${testUserId}`)
				.set("Authorization", `Bearer ${authToken}`);
			expect(res.statusCode).toEqual(500);
		...
		it("PUT /api/orgs/:orgId/users/:userId/roles - should return 404 for invalid roles", async () => {
			const spy = vi.spyOn(orgRoleService, 'setRolesForAssignment').mockRejectedValue(new Error("User is not a member of this organization"));
			const res = await request(app)
				.put(`/api/orgs/${testOrgId}/users/${testUserId}/roles`)
				.set("Authorization", `Bearer ${authToken}`)
				.send({ roles: ["invalid"] });
		...
		it("PUT /api/orgs/:orgId/users/:userId/roles - should handle generic errors", async () => {
			const spy = vi.spyOn(orgRoleService, 'setRolesForAssignment').mockRejectedValue(new Error("Some other error"));
			const res = await request(app)
				.put(`/api/orgs/${testOrgId}/users/${testUserId}/roles`)
				.set("Authorization", `Bearer ${authToken}`)
				.send({ roles: ["admin"] });
		...
		it("GET /api/orgs/my - should handle service errors", async () => {
			const spy = vi.spyOn(orgMembershipService, 'getUserOrganizations').mockRejectedValue(new Error("Database error"));
			const res = await request(app)
				.get("/api/orgs/my")
				.set("Authorization", `Bearer ${authToken}`);
			expect(res.statusCode).toEqual(500);
		...
		it("GET /api/orgs/:orgId/users - should return 404 for org not found", async () => {
			const spy = vi.spyOn(orgRoleService, 'getOrgUsers').mockRejectedValue(new Error("Organization not found"));
			const res = await request(app)
		...
		it("GET /api/orgs/:orgId/users - should handle generic errors", async () => {
			const spy = vi.spyOn(orgRoleService, 'getOrgUsers').mockRejectedValue(new Error("Some other error"));
			const res = await request(app)
				.get(`/api/orgs/${testOrgId}/users`)
				.set("Authorization", `Bearer ${authToken}`);
			expect(res.statusCode).toEqual(500);
		...
		it("GET /api/orgs/:orgId/users/role - should return 404 for role not found", async () => {
			const spy = vi.spyOn(orgRoleService, 'getUsersByRole').mockRejectedValue(new Error("Role not found"));
			const res = await request(app)
				.get(`/api/orgs/${testOrgId}/users/role`)
		...
		it("GET /api/orgs/:orgId/users/role - should handle generic errors", async () => {
			const spy = vi.spyOn(orgRoleService, 'getUsersByRole').mockRejectedValue(new Error("Some other error"));
			const res = await request(app)
				.get(`/api/orgs/${testOrgId}/users/role`)
				.query({ role: 'admin' })
				.set("Authorization", `Bearer ${authToken}`);
		...
		it("DELETE /api/orgs/:orgId/users/:userId - should return 404 for user not member", async () => {
			const spy = vi.spyOn(orgRoleService, 'removeUserFromOrg').mockRejectedValue(new Error("not a member"));
			const res = await request(app)
		...
		it("DELETE /api/orgs/:orgId/users/:userId - should handle generic errors", async () => {
			const spy = vi.spyOn(orgRoleService, 'removeUserFromOrg').mockRejectedValue(new Error("Some other error"));
			const res = await request(app)
		...
		it("DELETE /api/orgs/:orgId/users/:userId/roles - should return 404 for invalid role removal", async () => {
			const spy = vi.spyOn(orgRoleService, 'removeRolesFromUser').mockRejectedValue(new Error("required role"));
			const res = await request(app)
				.delete(`/api/orgs/${testOrgId}/users/${testUserId}/roles`)
				.set("Authorization", `Bearer ${authToken}`)
				.send({ role: "invalid" });
		...
		it("DELETE /api/orgs/:orgId/users/:userId/roles - should handle generic errors", async () => {
			const spy = vi.spyOn(orgRoleService, 'removeRolesFromUser').mockRejectedValue(new Error("Some other error"));
			const res = await request(app)
				.delete(`/api/orgs/${testOrgId}/users/${testUserId}/roles`)
				.set("Authorization", `Bearer ${authToken}`)
				.send({ role: "admin" });
		...
		it("POST /api/orgs/:orgId/invite - should return 400 for user already in org", async () => {
			const spy = vi.spyOn(orgMembershipService, 'inviteByEmail').mockRejectedValue(new Error("already in this organization"));
			const res = await request(app)
				.post(`/api/orgs/${testOrgId}/invite`)
				.set("Authorization", `Bearer ${authToken}`)
		...
		it("POST /api/orgs/:orgId/invite - should handle generic errors", async () => {
			const spy = vi.spyOn(orgMembershipService, 'inviteByEmail').mockRejectedValue(new Error("Some other error"));
			const res = await request(app)
				.post(`/api/orgs/${testOrgId}/invite`)
				.set("Authorization", `Bearer ${authToken}`)
		...
		it("PUT /api/orgs/:orgId/respond - should handle generic errors", async () => {
			const spy = vi.spyOn(models.OrgMembership, 'findOne').mockRejectedValue(new Error("Some other error"));
			const res = await request(app)
				.put(`/api/orgs/${testOrgId}/respond`)
				.set("Authorization", `Bearer ${authToken}`)
				.send({ action: "accept" });
		...
		it("POST /api/orgs/:orgId/join - should catch 'User already in organization' (Line 21)", async () => {
			const spy = vi.spyOn(orgMembershipService, 'addUserToOrg').mockRejectedValue(new Error("User already in organization"));
			const res = await request(app).post(`/api/orgs/${testOrgId}/join`).send({ userId: testUserId });
			expect(res.statusCode).toEqual(409);
			spy.mockRestore();
		});
		...
		it("GET /api/orgs/:orgId/users/role - should catch 'not found' (Line 81)", async () => {
			const spy = vi.spyOn(orgRoleService, 'getUsersByRole').mockRejectedValue(new Error("Role not found"));
			const res = await request(app).get(`/api/orgs/${testOrgId}/users/role?role=fake`);
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});

		it("DELETE /api/orgs/:orgId/users/:userId - should catch 'not a member' (Line 96)", async () => {
			const spy = vi.spyOn(orgRoleService, 'removeUserFromOrg').mockRejectedValue(new Error("User is not a member"));
			const res = await request(app).delete(`/api/orgs/${testOrgId}/users/${testUserId}`).set("Authorization", `Bearer ${authToken}`);
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});

		it("DELETE .../roles - should catch 'required' role error (Line 111)", async () => {
			const spy = vi.spyOn(orgRoleService, 'removeRolesFromUser').mockRejectedValue(new Error("Role is required"));
			const res = await request(app).delete(`/api/orgs/${testOrgId}/users/${testUserId}/roles`).set("Authorization", `Bearer ${authToken}`).send({ role: "admin" });
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});

		it("POST /api/orgs/:orgId/invite - should catch 'No user found' (Line 133)", async () => {
			const spy = vi.spyOn(orgMembershipService, 'inviteByEmail').mockRejectedValue(new Error("No user found with that email"));
			const res = await request(app).post(`/api/orgs/${testOrgId}/invite`).set("Authorization", `Bearer ${authToken}`).send({ email: "ghost@viu.ca" });
			expect(res.statusCode).toEqual(400);
			spy.mockRestore();
		});

		it("POST /api/orgs/:orgId/join - should hit next(error) (Line 21)", async () => {
			const spy = vi.spyOn(orgMembershipService, 'addUserToOrg').mockRejectedValue(new Error("Random Database Error"));
			const res = await request(app).post(`/api/orgs/${testOrgId}/join`).set("Authorization", `Bearer ${authToken}`).send({ userId: testUserId });
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});

		it("GET /api/orgs/:orgId/users/role - should hit next(error) (Line 81)", async () => {
			const spy = vi.spyOn(orgRoleService, 'getUsersByRole').mockRejectedValue(new Error("Random Database Error"));
			const res = await request(app).get(`/api/orgs/${testOrgId}/users/role?role=admin`).set("Authorization", `Bearer ${authToken}`);
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});

		it("DELETE /api/orgs/:orgId/users/:userId - should catch 'Organization not found' (Line 96)", async () => {
			const spy = vi.spyOn(orgRoleService, 'removeUserFromOrg').mockRejectedValue(new Error("Organization not found"));
			const res = await request(app).delete(`/api/orgs/${testOrgId}/users/${testUserId}`).set("Authorization", `Bearer ${authToken}`);
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});
		...
		it("DELETE .../roles - should catch 'not found' and 'does not' (Lines 111 & 112)", async () => {
			let spy = vi.spyOn(orgRoleService, 'removeRolesFromUser').mockRejectedValue(new Error("Role not found"));
			let res = await request(app).delete(`/api/orgs/${testOrgId}/users/${testUserId}/roles`).set("Authorization", `Bearer ${authToken}`).send({ role: "admin" });
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();

			spy = vi.spyOn(orgRoleService, 'removeRolesFromUser').mockRejectedValue(new Error("User does not have role"));
			res = await request(app).delete(`/api/orgs/${testOrgId}/users/${testUserId}/roles`).set("Authorization", `Bearer ${authToken}`).send({ role: "admin" });
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});

		it("POST /api/orgs/:orgId/invite - should catch 'already in' (Line 133)", async () => {
			const spy = vi.spyOn(orgMembershipService, 'inviteByEmail').mockRejectedValue(new Error("User is already in this org"));
			const res = await request(app).post(`/api/orgs/${testOrgId}/invite`).set("Authorization", `Bearer ${authToken}`).send({ email: "ghost@viu.ca" });
			expect(res.statusCode).toEqual(400);
			spy.mockRestore();
		});
	...
	describe("organizationController - Error Handling", () => {
		it("GET /api/orgs - should handle generic errors", async () => {
			const spy = vi.spyOn(organizationService, 'getAll').mockRejectedValue(new Error("Some error"));
			const res = await request(app).get("/api/orgs");
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		it("GET /api/orgs/:id - should return 404 for organization not found", async () => {
			const spy = vi.spyOn(organizationService, 'getById').mockRejectedValue(new Error("Organization not found"));
			const res = await request(app).get(`/api/orgs/999`);
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});
		it("GET /api/orgs/:id - should handle generic errors", async () => {
			const spy = vi.spyOn(organizationService, 'getById').mockRejectedValue(new Error("Some other error"));
			const res = await request(app).get(`/api/orgs/${testOrgId}`);
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		it("PUT /api/orgs/:id - should return 404 for organization not found", async () => {
			const spy = vi.spyOn(organizationService, 'update').mockRejectedValue(new Error("Organization not found"));
			const res = await request(app).put(`/api/orgs/${testOrgId}`).set("Authorization", `Bearer ${authToken}`).send({ name: "Updated" });
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});
		it("PUT /api/orgs/:id - should handle generic errors", async () => {
			const spy = vi.spyOn(organizationService, 'update').mockRejectedValue(new Error("Some other error"));
			const res = await request(app).put(`/api/orgs/${testOrgId}`).set("Authorization", `Bearer ${authToken}`).send({ name: "Updated" });
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		it("DELETE /api/orgs/:id - should return 404 for organization not found", async () => {
			const spy = vi.spyOn(organizationService, 'remove').mockRejectedValue(new Error("Organization not found"));
			const res = await request(app).delete(`/api/orgs/${testOrgId}`).set("Authorization", `Bearer ${authToken}`);
			expect(res.statusCode).toEqual(404);
			spy.mockRestore();
		});
		it("DELETE /api/orgs/:id - should handle generic errors", async () => {
			const spy = vi.spyOn(organizationService, 'remove').mockRejectedValue(new Error("Some other error"));
			const res = await request(app).delete(`/api/orgs/${testOrgId}`).set("Authorization", `Bearer ${authToken}`);
			expect(res.statusCode).toEqual(500);
			spy.mockRestore();
		});
		...
		it("getOrgUsers - should throw 'No users found' (Line 68)", async () => {
			...
			const emptyOrg = await models.Organization.create({ name: "Ghost Town Org" });
			
			await expect(orgRoleService.getOrgUsers(emptyOrg.id))
				.rejects.toThrow("No users found for this organization");
		});

		it("removeUserFromOrg - should throw 'Organization not found' (Line 126)", async () => {
			await expect(orgRoleService.removeUserFromOrg(99999, testUserId))
				.rejects.toThrow("Organization not found");
		});

		it("removeRolesFromUser - should throw 'Role names are required' (Line 137)", async () => {
			await expect(orgRoleService.removeRolesFromUser(testOrgId, testUserId, []))
		...
		it("removeRolesFromUser - should throw 'Membership or Roles not found' (Line 149)", async () => {
			...
			await expect(orgRoleService.removeRolesFromUser(testOrgId, 99999, ["admin"]))
				.rejects.toThrow("Membership or Roles not found");
				
			// Testing a fake role
			await expect(orgRoleService.removeRolesFromUser(testOrgId, testUserId, ["mythical-role"]))
				.rejects.toThrow("Membership or Roles not found");
		});
		...
		describe("organization.service.js - Direct Service Tests", () => {
			...
			it("create - should throw 'President role not found' (Line 30)", async () => {
				...
				const spy = vi.spyOn(models.Role, 'findOne').mockResolvedValue(null);
				
				await expect(organizationService.create({ name: "No President Org", userId: testUserId }))
			...
			it("create - should rollback transaction on error (Lines 47-48)", async () => {
				...
				const spy = vi.spyOn(models.Organization, 'create').mockRejectedValue(new Error("Critical DB Transaction Failure"));
				
				await expect(organizationService.create({ name: "Fail Org", userId: testUserId }))
