...
Given("an inventory item with ID {int} exists", (_itemId) => {
...
When(
	"they attempt to add a {string} to the {string} department in the show inventory",
	async function (itemName, department) {
		const deptIdMap = {
			Lighting: 1,
			Tech: 2,
			Costumes: 3,
...
When(
	"they pull inventory item ID {int} for Rock of Ages",
...
Then("the response should contain a list of inventory items", function () {
	...
	assert.ok(
		...
		"Expected the response to contain an array of inventory items",
	);
});
...
Then("the item should be saved successfully", function () {
	...
	assert.ok(
		...
		"Expected the database to return the newly created item ID",
	);
	assert.ok(
		this.lastResponse.body.data.name,
		"Expected the item to have a name",
	);
});
