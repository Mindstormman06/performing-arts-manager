...
export default async function seed() {
	try {
		...
		await models.ShowRole.bulkCreate([
			...
			{ id: 101, name: "stage-manager" },
			{ id: 102, name: "actor" },
			{ id: 103, name: "costumes" },
			{ id: 104, name: "props" },
			{ id: 105, name: "sets" },
			{ id: 106, name: "tech" },
		...
		await models.OrgRoleRelationship.bulkCreate([
			...
			{
				assignment_id: orgMemberships.find((m) => m.users_id === 1)
					.assignment_id,
				role_id: 7,
			}, // Aiden: Tech
		]);
		...
		const _shows = await models.Show.bulkCreate([
			{
				id: 1,
				title: "Rock of Ages",
				start_date: now,
			...
			},
			{
				...
				end_date: new Date(nextMonth.getTime() + 2592000000),
				organization_id: org.id,
			},
		]);
		...
		const roaMembers = await models.ShowMembership.bulkCreate(roaMembersData, {
			returning: true,
		});
		...
		await models.ShowRoleRelationship.bulkCreate([
			{
				assignment_id: roaMembers.find((m) => m.users_id === 4).assignment_id,
				role_id: 100,
			}, // Sarah: Director
			{
				assignment_id: roaMembers.find((m) => m.users_id === 2).assignment_id,
				role_id: 101,
			}, // Laura: Stage Manager
			{
				assignment_id: roaMembers.find((m) => m.users_id === 1).assignment_id,
				role_id: 107,
			}, // Aiden: Lighting Design
			{
				assignment_id: roaMembers.find((m) => m.users_id === 1).assignment_id,
				role_id: 106,
			}, // Aiden: Tech
			{
				assignment_id: roaMembers.find((m) => m.users_id === 5).assignment_id,
				role_id: 102,
			}, // Marcus: Actor
			{
				assignment_id: roaMembers.find((m) => m.users_id === 7).assignment_id,
				role_id: 102,
			}, // David: Actor
			{
				assignment_id: roaMembers.find((m) => m.users_id === 8).assignment_id,
				role_id: 102,
			}, // Rachel: Actor
			{
				assignment_id: roaMembers.find((m) => m.users_id === 3).assignment_id,
				role_id: 105,
			}, // Liam: Sets
		]);
		...
		const techWeek = new Date(now);
		techWeek.setDate(techWeek.getDate() + 14);

		await models.Schedule.bulkCreate([
			{
				id: 1,
				title: "Read-Through & Music Rehearsal",
				start_time: tomorrow,
				end_time: new Date(tomorrow.getTime() + 10800000),
				location: "Main Hall",
				description: "Full cast read-through.",
				show_id: 1,
				org_id: 1,
			},
			{
				id: 2,
				title: "Choreography: Don't Stop Believin'",
				start_time: inThreeDays,
				end_time: new Date(inThreeDays.getTime() + 7200000),
				location: "Dance Studio",
				description: "Wear comfortable clothes.",
				show_id: 1,
				org_id: 1,
			},
			{
				id: 3,
				title: "Lighting Plot Hang & Focus",
				start_time: techWeek,
				end_time: new Date(techWeek.getTime() + 18000000),
				location: "Main Stage",
				description: "All tech crew required.",
				show_id: 1,
				org_id: 1,
			},
		]);
		...
		await models.UserSchedule.bulkCreate([
			{ schedules_id: 1, users_id: 4 },
			{ schedules_id: 1, users_id: 2 },
			{ schedules_id: 1, users_id: 5 },
			{ schedules_id: 1, users_id: 7 },
			{ schedules_id: 1, users_id: 8 },
			{ schedules_id: 2, users_id: 4 },
			{ schedules_id: 2, users_id: 5 },
			{ schedules_id: 2, users_id: 8 },
			{ schedules_id: 3, users_id: 1 },
			{ schedules_id: 3, users_id: 2 },
			{ schedules_id: 3, users_id: 3 },
		]);
		...
		await models.Inventory.bulkCreate([
			...
			},
			{
				...
				description: "Studded 80s jacket.",
				dept_id: 3,
				is_global: 0,
				added_by: 4,
				org_id: 1,
			...
			{
				id: 5,
				name: "Fake Electric Guitar",
				description: "Prop Gibson Les Paul replica.",
				dept_id: 4,
				is_global: 0,
				added_by: 2,
				org_id: 1,
