...
async function join(req, res, next) {
    try {
        ...
        const membership = await showMembershipService.addUserToShow(showId, userId);
...
async function addRoles(req, res, next) {
    try {
        ...
        const updatedRoles = await showRoleService.appendRolesToAssignment(showId, userId, roles);
...
async function getAllUsers(req, res, next) {
    try {
        ...
        if (error.message.includes('No users found') || error.message.includes('Show not found')) {
            return res.status(404).json({ success: false, message: error.message });
        }
        next(error); 
    }
...
async function getUser(req, res, next) {
    try {
        const user = await showRoleService.getShowUserById(req.params.showId, req.params.userId);
        res.json(user);
    } catch (error) {
...
async function getByRole(req, res, next) {
    try {
        const users = await showRoleService.getUsersByRole(req.params.showId, req.query.role);
        res.json(users);
    } catch (error) { 
        if (error.message.includes('not found')) {
            return res.status(404).json({ success: false, message: error.message });
...
async function leave(req, res, next) {
    try {
        const result = await showRoleService.removeUserFromShow(req.params.showId, req.params.userId);
        res.status(200).json(result);
    } catch (error) {
        if (error.message.includes('not a member') || error.message.includes('Show not found')) return res.status(404).json({ success: false, message: error.message });
        next(error);
    }
}
...
async function removeRole(req, res, next) {
    try {
        ...
        const result = await showRoleService.removeRolesFromUser(showId, userId, rolesToRemove);
        res.json(result);
    } catch (error) {
